...
class ExportArticleVideo extends React.Component {
  ...
  componentDidMount() {
    const { action, skip } = queryString.parse(window.location.search);
  ...
  render() {
    const { fetchArticleVideoState, articleVideo, article } = this.props;
    if (!article) return <span>loading...</span>;

    // let initialFormValues = {
    //   ...UPLOAD_FORM_INITIAL_VALUES,
    //   sourceUrl: `${window.location.origin}/videowiki/${article.title}?wikiSource=${article.wikiSource}`,
    // };
    // let disabledFields = [];
    // let mode = 'new';
    ...
    //     addExtraUsers: false,
    //     extraUsers: [],
    //     sourceUrl: `${window.location.origin}/${this.props.language}/videowiki/${article.title}?wikiSource=${article.wikiSource}`,
    //   };
    ...
    if (fetchArticleVideoState === 'done' && articleVideo) {
      if (articleVideo.exported && articleVideo.video && (articleVideo.video.commonsUploadUrl || articleVideo.video.commonsUrl || articleVideo.video.url)) {
        options.push({
          text: (
            <a href={articleVideo.video.commonsUrl ? `${articleVideo.video.commonsUploadUrl || articleVideo.video.commonsUrl}?download` : articleVideo.video.url} target="_blank" rel="noopener noreferrer" >
              Download video
            </a>
          ),
          value: 'export',
